...
import PickupLocationNotice from './PickupLocationNotice';
import ShippingForm from './ShippingForm';
import ShippingHeader from './ShippingHeader';
import { getCustomerMessage } from './shoppingLocation';
...
function Shipping({
    ...
    const isPickup =
        ...
        consignments.every(({ selectedPickupOption }) =>
            Boolean(selectedPickupOption?.pickupMethodId),
        );
    ...
    useEffect(() => {
        const initializeShipping = async () => {
            try {
                ...
                if (hasCompanyAddressBook && !isPickup) {
    ...
    const handleSingleShippingSubmit = async (values: SingleShippingFormValues) => {
        ...
        if (
            ...
            (!isEqualAddress(updatedShippingAddress, shippingAddress) ||
                getShouldSaveAddress(shippingAddress) !==
                    getShouldSaveAddress(updatedShippingAddress))
        ) {
        ...
        if (customerMessage !== values.orderComment) {
            promises.push(
                updateCheckout({
                    customerMessage: getCustomerMessage(shoppingLocation, values.orderComment),
                }),
            );
        }
        ...
        try {
            ...
            navigateNextStep(isPickup ? false : values.billingSameAsShipping);
        } catch (error) {
            if (error instanceof Error) {
                onUnhandledError(error);
            }
    ...
    return (
        <AddressFormSkeleton isLoading={isInitializing} renderWhileLoading={true}>
            <div className="checkout-form">
                {isPickup && shoppingLocation && (
                    <PickupLocationNotice location={shoppingLocation} />
                )}
                <ConfirmationModal
