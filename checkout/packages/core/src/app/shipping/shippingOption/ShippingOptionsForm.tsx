...
const SALES_REP_QUOTE_METHOD = 'Sales Rep will provide shipping quote';
...
const ShippingOptionsForm = (
    ...
    const selectDefaultShippingOptions = async ({ data }: CheckoutSelectors) => {
        ...
        const salesRepQuoteOption = availableShippingOptions.find(
            ({ description }) => description === SALES_REP_QUOTE_METHOD,
        );
        const singleShippingOption =
            availableShippingOptions.length === 1 && availableShippingOptions[0];
        const defaultShippingOption =
            salesRepQuoteOption || recommendedOption || singleShippingOption;
