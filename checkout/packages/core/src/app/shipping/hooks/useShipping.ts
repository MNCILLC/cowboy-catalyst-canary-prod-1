...
import { getOrderComment, getShoppingLocation } from '../shoppingLocation';
...
export const useShipping = () => {
    ...
    const shoppingLocation = getShoppingLocation(checkout.customerMessage);
    ...
    const createPickupConsignment = useCallback(
        (pickupMethodId: number) =>
            checkoutService.createConsignments([
                {
                    pickupOption: { pickupMethodId },
                    lineItems: cart.lineItems.physicalItems.map(({ id, quantity }) => ({
                        itemId: id,
                        quantity,
                    })),
                },
            ]),
        [cart.lineItems.physicalItems, checkoutService],
    );
    ...
    return {
        ...
        customer,
        customerMessage: getOrderComment(checkout.customerMessage),
