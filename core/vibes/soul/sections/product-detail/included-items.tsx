...
export function IncludedItems({ items, title }: Props) {
  ...
  return (
    <section aria-label={title} className="bg-stone-50 @container">
      <div className="mx-auto w-full max-w-screen-2xl px-4 py-10 @xl:px-6 @xl:py-14 @4xl:px-8">
        ...
        <ul className="grid grid-cols-1 gap-4 @xl:grid-cols-2 @4xl:grid-cols-3 @4xl:gap-6">
          {items.map((item, index) => (
            <li
              className="flex min-w-0 items-center gap-4 rounded-2xl bg-white p-6 text-base leading-relaxed text-slate-900 shadow-sm @xl:text-lg"
              key={index}
            >
